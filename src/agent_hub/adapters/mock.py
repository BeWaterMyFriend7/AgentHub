...
class MockAgentAdapter(AgentAdapter):
    def __init__(self, tool, sessions: list[AgentSession]) -> None:
        super().__init__(tool)
    ...
    async def probe(self) -> ProbeResult:
        ...
        if not self.tool.enabled:
            failures.append("工具已被禁用。")
        if not self.tool.endpoint.strip():
        ...
        if failures:
            self.tool.connected = False
            self.tool.last_probe_message = "；".join(failures)
            return ProbeResult(
                ok=False,
                tool_id=self.tool.id,
                title="接入探测失败",
                message=f"{self.tool.name} 尚未达到完整接入标准。",
                checks=checks,
                failures=failures,
            )

        self.tool.connected = True
        self.tool.last_probe_message = "接入能力校验通过"
        return ProbeResult(
            ok=True,
            tool_id=self.tool.id,
            title="接入探测通过",
            message=f"{self.tool.name} 可以完成会话发现、状态读取和精确恢复。",
            checks=checks,
        )
    ...
    async def open_session(self, session_id: str) -> OpenSessionResult:
        ...
        if not self.tool.capabilities.exact_resume:
            return OpenSessionResult(
                ok=False,
                session_id=session_id,
                tool_id=self.tool.id,
                ...
                resume_target=self.tool.endpoint,
            )
        ...
        return OpenSessionResult(
            ...
            message=f"Demo：已调用 {self.tool.name} 适配器恢复“{session.title}”。",
            resume_target=f"{self.tool.id}://session/{session.native_session_id}",
        )
