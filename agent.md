...
- `docs/README.md`：产品定位、整体范围和文档导航。
- `docs/architecture/`：统一系统架构和需要长期保留的 ADR。
- `docs/modules/`：Agents、Sessions、Workflows、Capabilities、Operations 的需求、设计、测试和验证证据。
- `docs/process/`：路线图、当前待办和经验记录。
- `docs/prototypes/`：一次性 UI 或技术原型。
- `docs/guides/`：运维和开发操作指南。
- `scripts/`：仓库维护、迁移和验证脚本。

开始工作前先阅读 `docs/README.md`。不得把原型、历史参考或验证记录当作当前需求事实来源。
...
6. `docs/README.md` 说明产品定位和整体范围；模块 `requirements.md` 维护所属需求事实；设计说明如何实现；ADR 记录难以逆转的选择；BDD 表达复杂且稳定的可观察行为；Verification 记录真实验证证据。它们不得互相替代。
7. 代码变更导致产品范围、模块需求、设计、ADR、BDD、验证证据或 TODO 失效时，应在同一次变更中同步更新。
...
12. 所有 Git Commit 信息必须使用中文，并采用“总结信息 + 详情列表”格式：首行填写简洁的中文总结，空一行后使用 `- ` 列出具体改动。例如：
